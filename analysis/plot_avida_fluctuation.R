...
treatments <- read_csv(file = "data/fluctuation_treatments.csv",
                       col_types = list(Treatment = col_factor(levels = c("10-40-10", "18-20-18", "20-18-20", "40-10-40")),
                                        Resource = col_factor(levels = c(10, 18, 20, 40)))) %>%
    mutate(Update = 0, Frac_Cooperators = 0) %>%
    filter(Treatment %in% c("10-40-10", "40-10-40"))

d <- read_csv(file = "data/fluctuation_cooppct.csv",
              col_types = list(Treatment = col_factor(levels = c("10-40-10", "18-20-18", "20-18-20", "40-10-40")),
